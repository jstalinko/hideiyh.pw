  ...
  <div class="bg-gray-900 text-white font-sans antialiased">
    ...
    <header class="bg-gray-900/70 backdrop-blur-lg sticky top-0 z-50 border-b border-gray-800">
      <div class="container mx-auto px-6 py-4 flex justify-between items-center">
        <a href="#" class="text-2xl font-bold text-orange-500">
          <img src="/logo-text.png" alt="Logo" class="h-12 w-auto">
        </a>
        ...
        <div class="hidden md:block">
          <button @click="goadmin"
            class="bg-orange-500 hover:bg-orange-600 text-white font-bold py-2 px-6 rounded-lg shadow-md transition-transform transform hover:scale-105">
            Login
          </button>
        </div>
      ...
      <div v-show="isMobileMenuOpen" class="md:hidden px-6 pb-4 space-y-2">
        ...
        <a href="#testimonials" @click="isMobileMenuOpen = false"
          class="block py-2 hover:text-orange-400">Testimonials</a>
        <a href="#faq" @click="isMobileMenuOpen = false" class="block py-2 hover:text-orange-400">FAQ</a>
        <button @click="goadmin"
          class="w-full mt-2 bg-orange-500 hover:bg-orange-600 text-white font-bold py-2 px-6 rounded-lg">Login</button>
      </div>
    </header>
        ...
        <div class="container mx-auto px-6 relative z-10 grid md:grid-cols-2 gap-12 items-center">
          <div class="text-center md:text-left">
            ...
            <p class="text-lg md:text-xl text-gray-300 max-w-xl mx-auto md:mx-0 mb-10">
              Platform cloaking canggih untuk melindungi kampanye Anda dari bot, reviewer, dan trafik yang tidak
              diinginkan. HideIyh.pw menyediakan API deteksi bot, SmartURL, dan dashboard web yang intuitif untuk
              mengelola domain serta tautan Anda. Dirancang khusus agar lolos dari peninjauan Meta Ads & Google Ads,
              mendukung geotargeting, dan siap digunakan untuk bisnis skala kecil hingga besar.
            </p>
            <div class="flex flex-col sm:flex-row gap-4 justify-center md:justify-start">
              <a href="#pricing"
                class="bg-orange-500 hover:bg-orange-600 text-white font-bold py-3 px-8 rounded-lg shadow-lg text-lg transition-transform transform hover:scale-105">
                Lihat Harga
              </a>
              <button @click="goadmin"
                class="bg-gray-700 hover:bg-gray-600 text-gray-200 font-bold py-3 px-8 rounded-lg text-lg transition-colors">
                Login
              </button>
            </div>
          </div>
          <div class="flex justify-center items-center">
            <svg viewBox="0 0 200 200" class="w-full max-w-sm h-auto">
              ...
              <path d="M 100,20 A 80,80 0 0,1 100,180" fill="none" stroke="#dd6b20" stroke-width="4"
                stroke-linecap="round">
                <animateTransform attributeName="transform" type="rotate" from="0 100 100" to="360 100 100" dur="10s"
                  repeatCount="indefinite" />
              </path>
              <g class="magnifying-glass">
                <circle cx="85" cy="85" r="25" fill="none" stroke="#dd6b20" stroke-width="3" filter="url(#glow)" />
                <line x1="105" y1="105" x2="120" y2="120" stroke="#dd6b20" stroke-width="4" stroke-linecap="round"
                  filter="url(#glow)" />
              </g>
              <g fill="#4a5568">
      ...
      <section id="features" class="py-20 bg-gray-950">
        <div class="container mx-auto px-6">
          <div class="text-center mb-16">
            ...
            <p class="text-gray-400 max-w-2xl mx-auto">Semua yang Anda butuhkan untuk menjalankan kampanye iklan yang
              aman dan efisien.</p>
          </div>
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            <div v-for="feature in features" :key="feature.name"
              class="bg-gray-800/50 p-6 rounded-xl border border-gray-700 hover:border-orange-500/50 hover:-translate-y-2 transition-all duration-300 group">
          ...
          <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-center">
            <div v-for="plan in pricingPlans" :key="plan.name"
              ...
              <div v-if="plan.highlighted"
                class="absolute top-0 -translate-y-1/2 left-1/2 -translate-x-1/2 bg-orange-500 text-white text-xs font-bold px-4 py-1 rounded-full uppercase tracking-wider">
                Most Popular</div>
              ...
              <div class="my-5">
                <div class="flex items-baseline justify-center">
                  ...
                  <span class="text-4xl font-extrabold" :class="{ 'text-3xl': plan.highlighted }">{{
                    formatCurrency(plan.price) }}</span>
                  <span class="ml-1 text-xl font-semibold text-gray-400">/{{ plan.billing_cycle }}</span>
                </div>
                <div class="text-center h-6 mt-1"> <span v-if="plan.originalPrice"
              ...
              <a :href="'/plan/' + plan.name.toLowerCase()" target="_blank"
                :class="['w-full mt-auto font-bold py-3 px-6 rounded-lg transition-colors', plan.highlighted ? 'bg-orange-500 hover:bg-orange-600 text-white' : 'bg-gray-700 hover:bg-gray-600 text-gray-200']">
                Pilih Paket
              </a>
            </div>
          </div>
        ...
        <div class="container mx-auto px-6">
          ...
          <div class="relative">
            <div ref="sliderContainer"
              class="flex overflow-x-auto snap-x snap-mandatory scroll-smooth scrollbar-hide -mx-3">
              <div v-for="(testimonial, index) in testimonials" :key="index"
                class="flex-shrink-0 w-full md:w-1/4 snap-center p-3">
            ...
            <button @click="scrollTestimonials(-1)"
              class="absolute top-1/2 -translate-y-1/2 left-0 -translate-x-4 bg-gray-700 hover:bg-orange-500 rounded-full p-2 text-white z-10 hidden md:block transition-colors">
              <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path>
              </svg>
            </button>
            <button @click="scrollTestimonials(1)"
              class="absolute top-1/2 -translate-y-1/2 right-0 translate-x-4 bg-gray-700 hover:bg-orange-500 rounded-full p-2 text-white z-10 hidden md:block transition-colors">
              <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div class="space-y-4">
            <div v-for="(faq, index) in faqs" :key="index"
              ...
              <button @click="toggleFaq(index)"
                class="w-full text-left p-6 flex justify-between items-center hover:bg-gray-700/50 focus:outline-none transition-colors">
              ...
              <div class="overflow-hidden transition-all duration-500 ease-in-out"
                :style="{ maxHeight: activeFaqIndex === index ? '200px' : '0px' }">
    ...
    <footer class="bg-gray-950 border-t border-gray-800">
      <div class="container mx-auto px-6 py-8 text-center text-gray-400">
        ...
        <p class="text-sm mt-2">Powered by <a href="https://javaradigital.com" target="_blank"
            class="underline">Javaradigital</a></p>
      </div>
    </footer>
  </div>
</template>
